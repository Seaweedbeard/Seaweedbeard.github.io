...
function storeKeys(type) {
  if (type == "initial") {
    setTimeout(function () {
      if (!localStorage.getItem("P1movementKeys") && !localStorage.getItem("P2movementKeys") && !localStorage.getItem("extraKeys")) {
...
function setup() {
  if (player2) {
    ...
    for (var keys in P1movementKeys) {
      document.getElementById("p1keys").innerHTML += P1movementKeys[keys] + " ";
    }

    document.getElementById("p2keys").innerHTML = "";

    for (var _keys in P2movementKeys) {
      if (P2movementKeys[_keys].length > 1) {
        var newString = P2movementKeys[_keys].substring(5);

        document.getElementById("p2keys").innerHTML += newString + " ";
      } else {
        document.getElementById("p2keys").innerHTML += P2movementKeys[_keys] + " ";
      }
    }
    ...
    player2.facing = "left";
    var pickbackground = Math.floor(Math.random() * backgrounds.length);
...
document.onkeydown = function (evt) {
  if (document.getElementById("pausemenu").style.display === "none") {
    if (!player1.dead && !player2.dead) {
      var _key = evt.key;
      _key = _key.toLowerCase(); //console.log(key);

      if (_key === P2movementKeys.left) {
        left = true;
        player2.facing = "left";
      }

      if (_key === P2movementKeys.right) {
        right = true;
        player2.facing = "right";
      }

      if (_key === P2movementKeys.up) {
        if (!P2airborne) {
          p2Jumping = true;
          P2airborne = true;
          jump("P2");
      ...
      if (_key === P2movementKeys.down) {
        down = true;
        crouch();
      }

      if (_key === P1movementKeys.left) {
        a = true;
        player1.facing = "left";
      }

      if (_key === P1movementKeys.right) {
        d = true;
        player1.facing = "right";
      }

      if (_key === P1movementKeys.up) {
        if (!P1airborne) {
          p1Jumping = true;
          P1airborne = true;
          jump("P1");
      ...
      if (_key === P1movementKeys.down) {
        s = true;
        crouch();
      }

      if (_key === P1movementKeys.block) {
        if (!p1blocking) {
          p1blocking = true;
        }
      }

      if (_key === P1movementKeys.punch) {
      ...
      if (_key === P2movementKeys.block) {
        if (!p2blocking && !p2kicking && !p2punching) {
          p2blocking = true;
        }
      }

      if (_key === extraKeys.addTime) {
        time = 100;
      }
    }
  }
...
function punchAttack(who) {
  if (who === "P1") {
    if (!p1Attacking) {
      if (P2block.active && player2.facing === "left" && player1.x + player1.width / 2 < player2.x + player2.width / 2 || P2block.active && player2.facing === "right" && player1.x + player1.width / 2 > player2.x + player2.width / 2) {
  ...
  if (who === "P2") {
    if (!p2Attacking) {
      if (P1block.active && player1.facing === "left" && player2.x + player2.width / 2 < player1.x + player1.width / 2 || P1block.active && player1.facing === "right" && player2.x + player2.width / 2 > player1.x + player1.width / 2) {
...
function gravity() {
  if (P1airborne) {
    if (player1.y <= ground.y - player1.height) {
      ...
      if (player1.y + player1.height > player2.y && player1.y + player1.height < player2.y + 10 && player1.yv > 0 && player1.x + player1.width > player2.x && player1.x < player2.x + player2.width) {
        down = true;
        bonk.play();
        crouch();
        setTimeout(function () {
  ...
  if (P2airborne) {
    if (player2.y <= ground.y - player2.height) {
      ...
      if (player2.y + player2.height > player1.y && player2.y + player2.height < player1.y + 10 && player2.yv > 0 && player2.x < player1.x + player1.width && player2.x + player2.width > player1.x) {
        s = true;
        bonk.play();
        crouch();
        setTimeout(function () {
