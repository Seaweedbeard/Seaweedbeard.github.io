...
function drawfrog() {
  ...
  for (var i = 0; i < buggy.length; i++) {
    ...
    ctx.translate(-buggy[i].x - 20, -buggy[i].y - 20); //bulldozer[i].draw();

    var bulldozers = new Image();
    ...
    ctx.translate(-racecar[i].x - 20, -racecar[i].y - 20); //truck[i].draw();

    var trucks = new Image();
  ...
  for (var _i = 0; _i < turtleslong.length; _i++) {
    ...
    turtlesshort[_i].draw();
  }

  for (var _i2 = 0; _i2 < logtop.length - 1; _i2++) {
    logtop[_i2].draw();
  }

  for (var _i3 = 0; _i3 < logmid.length; _i3++) {
  ...
  if (winningfrog) {
    ...
    for (var _i4 = 0; _i4 < winningfrog.length; _i4++) {
      winningfrog[_i4].draw();
    }
  } //frog.draw();
  //froginner.draw();
...
document.onkeydown = function (e) {
  ...
  if (frog.dead == false) {
    if (movingfrog == false) {
      ...
      }, 80); //Har en timeout som hindrer frosken i å bevege seg for raskt, og har en liten
...
function checkfrogposition() {
  ...
  for (var _i5 = 0; _i5 < 5; _i5++) {
    if (frog.y < 1 * 46.7) {
      if (frog.x >= _i5 * 3 * 46.7 - 20 && frog.x <= _i5 * 3 * 46.7 + 23.35) {
        deadByTurtle = 30;
        winfrog(_i5 * 3 * 46.7 - 5, _i5 * 3 * 46.7 + 46.7);
      } else {
        deadByTurtle -= 1;
...
function killfrog() {
  ...
  if (frog.dead == false) {
    console.error("Uncaught ReferenceError:    " + "You get no bitches (" + "%cfrogger.js:53:62" + "%c)", "color: white;", "");
    frog.dead = true;
    frog.x = 700;
    frog.y = 700;
    health--;
...
function winfrog(left, right) {
  ...
  if (winningfrog.length > 0) {
    for (var i = 0; i < winningfrog.length; i++) {
...
function adaptSpeed() {
  ...
  var loop = function loop() {
    ...
    if (now > 1000 + lastTime) {
      fps = Number(Math.round(frame * 1000 / (now - lastTime))); //console.log('1s, FPS:', fps);
...
function checkForBug() {
  ...
  if (turtleslong[0].x == turtleslong[1].x) {
    reset();
    var Anus = Math.floor(Math.random() * 3) + 1;
...
function movewaterstuff() {
  ...
  for (var i = 0; i < turtleslong.length; i++) {
  ...
  for (var _i7 = 0; _i7 < logbottom.length; _i7++) {
    logbottom[_i7].x += .5 * (60 / fps) * dickButt;

    if (logbottom[_i7].x > 15 * 46.7 + 6.7 * 2) {
      logbottom[_i7].x = -140.1;
    }
  }

  for (var _i8 = 0; _i8 < turtlesshort.length; _i8++) {
    turtlesshort[_i8].x -= .8 * (60 / fps) * dickButt;

    if (turtlesshort[_i8].x < -90) {
      turtlesshort[_i8].x = 16 * 46.7 + 6.7 * 2;
    }
  }

  for (var _i9 = 0; _i9 < logmid.length; _i9++) {
    logmid[_i9].x += 1 * (60 / fps) * dickButt;
  ...
  for (var _i10 = 0; _i10 < logtop.length; _i10++) {
    logtop[_i10].x += 1.2 * (60 / fps) * dickButt;

    if (logtop[_i10].x > 600) {
      logtop[_i10].x = -186.1;
    }
  }
